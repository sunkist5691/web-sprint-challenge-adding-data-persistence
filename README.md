...
`Relational database is a database that is relation each other having column and row`
`SQL is a structured query language that let us to communicate with database.`

2. Why do tables need a `primary key`?
`Primary key helps to give a unique id for each row and also can reference to other table`
